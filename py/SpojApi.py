    ...
    def get_sub_results(self,sub_id):
        ...
        if (row_data['result'] == self.result_strings['compile_error'] ):            
            ...
            if (row_data['result'] == "runtime error"):
                in_parenth=re.compile(r"\(([A-Za-z_ ].*)\)")
                match=in_parenth.match(status_cell.contents[0])
                if (match):
                    row_data['result-link']="<span>"+match.group()+"</span>"
                else:    
                    try:
                        row_data['result_link']=status_cell.contents[1].prettify()
                    except:
                        None
